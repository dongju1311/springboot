...
public class MemberController {
    ...
    public String idcheck(@RequestBody MemberDto memberDto) {
        boolean result = memberService.idCheck(memberDto.getId());  //아이디 O: 1, X: 0
        String msg = "";
    ...
    public boolean signup(@RequestBody MemberDto memberDto) {
        ...
        int rows = memberService.signup(memberDto);
    ...
    public ResponseEntity<?> login(@RequestBody MemberDto memberDto, HttpServletRequest request, HttpServletResponse response) {
        try {
            ...
            Authentication authenticationRequest =
                    UsernamePasswordAuthenticationToken.unauthenticated(memberDto.getId(), memberDto.getPwd());
            //2. 인증처리
            Authentication authenticationResponse =
                    this.authenticationManager.authenticate(authenticationRequest);
            System.out.println("인증 성공: " + authenticationResponse.getPrincipal());
            System.out.println("인증 성공: " + authenticationResponse.getAuthorities());
